...
SET MAINCLASSSRC=src/cs22/Game.java
SET MAINCLASSBIN=cs22.Game
SET IMAGES=img
SET RESOURCES=
...
mkdir "%BINDIR%"
mkdir "%BINDIR%\%SRCDIR%\%PACKAGE%"
copy "%SRCDIR%\%PACKAGE%\*.png" "%BINDIR%\%SRCDIR%\%PACKAGE%"
copy "%SRCDIR%\%PACKAGE%\*.gif" "%BINDIR%\%SRCDIR%\%PACKAGE%"

ECHO "1. Compiling ......................"
::javac -Xlint -cp ".;src;/SOFT/copy/dev/java/javafx/lib/*;/SOFT/COPY/db/derby/lib/*" src/Lab.java -d bin 2> labs-javac.err
javac --release 17 -Xlint -cp ".;%SRCDIR%" %MAINCLASSSRC% -d %BINDIR% 2> %BINERR%
